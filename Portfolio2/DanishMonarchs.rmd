...
- The functions including a dot (.) is a part of the normal Rpackage while, the underscore (_) is a tidyverse function, so this means kings2 and kings4 are tidyverse functions.
The difference between _CSV2 versus CSV is how they seperate commas (,) and semicolons (;).
My data is seperated by semicolon so I'll use kings4 (_CSV2).


2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
- We get a tibble that also behaves like a dataframe, it contains 55 rows and 7 collums.

3. How many columns does the object have when created with these different functions?
- We end up getting 7 columns which also matches with our number of variables in the 'Environment' section.

4. Show the dataset so that we can see how R interprets each column
- We used glimpse to show the dataset, but you can also use view or summarise for the same results.
...
```{r}
kings %>%
  filter(!is.na(End_year_reign&Start_year_reign)) %>%
...
```{r}
kings <- kings %>%
  mutate(duration = as.numeric(End_year_reign)-as.numeric(Start_year_reign))
...
```{r}
kings %>%
  ggplot(aes(x=Start_year_reign, y=duration)) +
  geom_point() +
  geom_line() +
  labs(title = "Duration of Reign Through Time",
       x = "Start of year reign",
       y = "Duration of reign") + 
  theme_minimal()
```
